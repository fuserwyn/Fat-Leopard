...
Я ваш хладнокровный тренер. Я всё вижу и не оставляю в стае тех, кто не тренируется — на 8-й день без отчёта удаляю из чата.

💪 Отчёты о тренировке:
• #training_done — засчитать тренировку (любое сообщение с этим тегом)

🏥 Больничный:
• #sick_leave — взять больничный (ставит таймер на паузу)
...
⏰ Как работает таймер:
• Таймер уже запущен — у тебя есть 7 дней до обнуления XP
• Каждый #training_done перезапускает таймер
• День 5 без отчёта — первое предупреждение (придёт в личку)
• День 6 без отчёта — второе предупреждение (придёт в личку)
• День 7 без отчёта — 🔴 XP обнуляется, последний шанс
• День 8 без отчёта — удаление из чата

🔥 Как копятся калории (XP):
• За 1-ю тренировку в серии — +1 калория, за 2-ю подряд — +2, за 3-ю — +3 и так далее (растёт с длиной серии)
• +2 бонусных калории в день, когда вернулся после #sick_leave через #healthy
• Пропустил день (серия сломалась) — счётчик начинается заново с 1

🏆 Кубки:
• +1 кубок за каждую тренировку (и за первую в день, и за дополнительные)
...
• 🏆🏆🏆🏆🏆🏆🏆🏆🏆🏆 180 дней подряд = +420 КУБКОВ
• 🏆🏆🏆🏆🏆🏆🏆🏆🏆🏆🏆 200 дней подряд = +4200 КУБКОВ
• 🏆🏆🏆🏆🏆🏆🏆🏆🏆🏆🏆🏆 240 дней подряд = +4200 КУБКОВ

📋 Коротко:
• Отчётом считается любое сообщение с тегом #training_done
• Заболел — #sick_leave, выздоровел — #healthy
• Предупреждения на днях 5 и 6 приходят в личные сообщения с ботом, поэтому обязательно открой со мной диалог: нажми /start в личке
• На 7-й день XP обнуляется, на 8-й — удаление из чата
...
func welcomeStartText() string {
	return `Добро пожаловать в стаю! 🦁
...
⏰ Как работает таймер:
• У тебя есть 7 дней до обнуления XP
• Каждый #training_done перезапускает таймер
• День 5 без отчёта — первое предупреждение (в личку)
• День 6 без отчёта — второе предупреждение (в личку)
• День 7 без отчёта — XP обнуляется, последний шанс
• День 8 без отчёта — удаление из чата

🔥 Как копятся калории (XP):
• За 1-ю тренировку в серии — +1 калория, за 2-ю подряд — +2, за 3-ю — +3 и так далее
• +2 бонусных калории в день, когда вернулся после #sick_leave через #healthy
• Если серия сломалась — счётчик снова начинается с 1

🏆 Кубки:
• +1 кубок за каждую тренировку
• 7 дней подряд = +42 кубка
• 14 дней подряд = +42 кубка
• 21 день подряд = +42 кубка
• 30 дней подряд = +420 кубков
...
• Отчётом считается любое сообщение с тегом #training_done
• Заболел — #sick_leave, выздоровел — #healthy
• Предупреждения на днях 5 и 6 приходят в личные сообщения, поэтому открой диалог с ботом: /start в личке
• На 7-й день XP обнуляется, на 8-й — удаление из чата
