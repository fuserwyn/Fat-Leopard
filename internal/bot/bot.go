			...
			if b.aiClient != nil {
				...
				if chatType == "writing" {
					doubleTrainingPrompts = []string{
						"Напиши одно связное сообщение (2-3 предложения) после дополнительной писательской сессии в тот же день: первое — отметь упорство и писательскую работу, второе — короткое практическое замечание по писательству. НЕ упоминай тренировки, спорт или физическую активность. НЕ используй фразы 'помни', 'дух', 'воля', 'преданность делу'. Не повторяй цифры. КРИТИЧЕСКИ ВАЖНО: отвечай ТОЛЬКО на этот отчёт. Используй ТОЛЬКО детали из сообщения. НЕ выдумывай детали. НЕ используй контекст других сообщений. Без Markdown.",
						"Сделай одно цельное сообщение (2-3 предложения) после второй писательской сессии за день: первое — похвали за энергию и отметь писательскую работу, второе — конкретное наблюдение по писательству. НЕ упоминай тренировки, спорт или физическую активность. Избегай фраз 'помни', 'дух', 'воля', 'дисциплина'. КРИТИЧЕСКИ ВАЖНО: отвечай ТОЛЬКО на этот отчёт. Без Markdown.",
						"Напиши одно связное сообщение (2-3 предложения): двойная писательская сессия — это особое вдохновение! Первое — отметь это и писательскую работу, второе — короткий комментарий по писательству. НЕ упоминай тренировки, спорт или физическую активность. НЕ используй абстрактные фразы про 'дух', 'волю', 'помни', 'преданность'. КРИТИЧЕСКИ ВАЖНО: используй ТОЛЬКО этот отчёт. Без Markdown.",
						"Сделай одно цельное сообщение (2-3 предложения) после дополнительной писательской сессии: первое — энергия впечатляет, отметь писательскую работу, второе — практический совет по писательству. НЕ упоминай тренировки, спорт или физическую активность. Избегай фраз 'помни', 'дух', 'воля', 'приближает к цели'. КРИТИЧЕСКИ ВАЖНО: отвечай ТОЛЬКО на этот отчёт. Без Markdown.",
					}
				} else {
					doubleTrainingPrompts = []string{
						"Напиши одно связное сообщение (2-3 предложения) после дополнительной тренировки в тот же день: первое — отметь упорство и упражнения, второе — короткое практическое замечание. НЕ используй фразы 'помни', 'дух', 'воля', 'преданность делу'. Не повторяй цифры. КРИТИЧЕСКИ ВАЖНО: отвечай ТОЛЬКО на этот отчёт. Используй ТОЛЬКО упражнения из сообщения. НЕ выдумывай детали. НЕ используй контекст других сообщений. Без Markdown.",
						"Сделай одно цельное сообщение (2-3 предложения) после второй тренировки за день: первое — похвали за энергию и отметь упражнения, второе — конкретное наблюдение. Избегай фраз 'помни', 'дух', 'воля', 'дисциплина'. КРИТИЧЕСКИ ВАЖНО: отвечай ТОЛЬКО на этот отчёт. Без Markdown.",
						"Напиши одно связное сообщение (2-3 предложения): двойная тренировка — это особое упорство! Первое — отметь это и упражнения, второе — короткий комментарий. НЕ используй абстрактные фразы про 'дух', 'волю', 'помни', 'преданность'. КРИТИЧЕСКИ ВАЖНО: используй ТОЛЬКО этот отчёт. Без Markdown.",
						"Сделай одно цельное сообщение (2-3 предложения) после дополнительной тренировки: первое — энергия впечатляет, отметь упражнения, второе — практический совет. Избегай фраз 'помни', 'дух', 'воля', 'приближает к цели'. КРИТИЧЕСКИ ВАЖНО: отвечай ТОЛЬКО на этот отчёт. Без Markdown.",
					}
				}
				now := utils.GetMoscowTime()
...
func (b *Bot) getUnifiedTrainingPrompt(streakDays, totalCalories, totalCups int, wasOnSickLeave bool, chatType string) string {
	...
	if chatType == "writing" {
		...
		// Базовые стили единых промптов для тренировок - отвечай ТОЛЬКО на этот отчёт
		prompts = []string{
			"Напиши одно связное сообщение (2-3 предложения) после отчёта #training_done: дружелюбно, но строго, как тренер. Первое предложение — конкретный комментарий к упражнениям из сообщения, второе — короткое наблюдение или совет. НЕ используй фразы типа 'помни', 'укрепление духа', 'сила воли'. НЕ повторяй цифры из сообщения, не перечисляй правила. КРИТИЧЕСКИ ВАЖНО: отвечай ТОЛЬКО на этот отчёт о тренировке. НЕ используй историю чата, последние сообщения или сообщения других участников. Используй ТОЛЬКО упражнения и детали из этого сообщения. НЕ выдумывай детали. Без Markdown.",

			"Сделай одно цельное сообщение (2-3 предложения) после тренировки: первое — отметь конкретные упражнения из сообщения, второе — короткий практический совет или наблюдение. Избегай общих фраз про 'дух', 'волю', 'помни'. Будь конкретным про упражнения, но не упоминай цифры. КРИТИЧЕСКИ ВАЖНО: отвечай ТОЛЬКО на этот отчёт. НЕ смотри на другие сообщения в чате. Без Markdown.",

			"Напиши одно связное сообщение (2-3 предложения) от лица строгого, но справедливого тренера Fat Leopard: первое — оценка тренировки с упоминанием упражнений, второе — конкретное замечание или совет. НЕ используй абстрактные фразы про 'дух', 'волю', 'помни'. КРИТИЧЕСКИ ВАЖНО: отвечай ТОЛЬКО на этот отчёт. Используй ТОЛЬКО упражнения из этого сообщения. НЕ смешивай с контекстом других сообщений. Без Markdown.",

			"Сделай одно цельное сообщение (2-3 предложения) после #training_done: первое — поддерживающий комментарий про конкретные упражнения, второе — практический совет или наблюдение. Избегай фраз 'помни', 'укрепление', 'дух', 'воля'. Не повторяй цифры. КРИТИЧЕСКИ ВАЖНО: используй ТОЛЬКО этот отчёт. НЕ используй сообщения других участников или историю чата. Без Markdown.",

			"Напиши одно связное сообщение (2-3 предложения) в стиле наставника: первое — конкретный комментарий к упражнениям из сообщения, второе — короткое практическое замечание. НЕ используй общие фразы про 'дух', 'волю', 'помни'. Будь конкретным, но не упоминай цифры. КРИТИЧЕСКИ ВАЖНО: отвечай ТОЛЬКО на этот отчёт. Без Markdown.",

			"Сделай одно цельное сообщение (2-3 предложения) после тренировки: первое — отметь упражнения из сообщения, второе — короткий комментарий о технике или подходе. Избегай абстрактных фраз про 'дух', 'волю', 'помни'. КРИТИЧЕСКИ ВАЖНО: используй ТОЛЬКО этот отчёт. НЕ смотри на последние сообщения. Без Markdown.",

			"Напиши одно связное сообщение (2-3 предложения): первое — конкретная оценка упражнений из сообщения, второе — короткое замечание о процессе тренировки. НЕ используй фразы 'помни', 'укрепление духа', 'сила воли'. Будь конкретным про упражнения, не упоминай цифры. КРИТИЧЕСКИ ВАЖНО: отвечай ТОЛЬКО на этот отчёт. Без Markdown.",

			"Сделай одно цельное сообщение (2-3 предложения) после #training_done: первое — комментарий к упражнениям, второе — короткое наблюдение о тренировке. Избегай общих фраз про 'дух', 'волю', 'помни'. КРИТИЧЕСКИ ВАЖНО: используй ТОЛЬКО этот отчёт. НЕ используй контекст других сообщений или участников. Без Markdown.",
		}
	}
