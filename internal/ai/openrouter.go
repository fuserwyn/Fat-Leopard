...
- Если в контексте указан пол пользователя (Пол: male/female), используй правильные формы: для female используй "начала", "молодец" (женский род), для male — "начал", "молодец" (мужской род). Если пол не указан, используй нейтральные формы
...
- Если пользователь даёт обратную связь (тон, обращение, пол, формат) — извинись кратко, скорректируйся и НЕ добавляй нерелевантные детали про тренировки.
- Упоминай тренировки/статистику только при явном запросе.
- Если спрашивают про пол (свой или другого пользователя) — отвечай кратко: "мужской" или "женский", без лишних слов типа "указан как" или "не указан".
...
Задача: Сформулируй «мудрость дня» — 3–5 коротких предложений на русском о силе духа, дисциплине и спорте. Без обращений к конкретным людям, без перечислений, без хэштегов и эмодзи, без Markdown. Не упоминай, что ты леопард или «монах». Всегда пиши новую формулировку — избегай повторов прошлых дней.`
	...
	userPrompt := fmt.Sprintf("Сегодня: %s. Тема дня: %s. Сгенерируй мудрость дня.", today, theme)
