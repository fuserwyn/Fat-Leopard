...
export function NewWorkoutScreen({
  ...
  /** Выбранное фото на экране предпросмотра (обрезка — по желанию). */
  ...
  useEffect(() => {
    ...
    const onStart = (e: TouchEvent) => {
      ...
      // Экран фото / обрезка — свой жест перетаскивания, не закрывать шторку.
      ...
      <div className="nwo__body" ref={bodyRef}>
        <div className="nwo__note-block">
          ...
          <textarea
            ...
            spellCheck
            onPaste={(e) => {
              // Картинка из буфера — экран предпросмотра (обрезка по желанию).
            ...
            <div className="nwo__photo-row">
              ...
              {photo ? (
                <>
                  ...
                  <button
                    type="button"
                    className="nwo__photo-edit nwo__photo-edit--icon"
                    onClick={() => setPendingCrop(photo)}
                    aria-label="Обрезать фото"
                    title="Обрезать фото"
                  >
                    <svg className="nwo__photo-crop-icon" viewBox="0 0 24 24" aria-hidden focusable="false">
      ...
      {PHOTO_ENABLED && pendingCrop
        ? createPortal(
            <PhotoCropper
              ...
              onReplace={(newFile) => {
                if (pendingPhoto) {
                  setPendingPhoto(newFile);
                  setPendingCrop(null);
                } else {
                  setPendingCrop(newFile);
                }
              }}
            />,
            document.body,
